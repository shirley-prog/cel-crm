...
const C = {
  ...
  offWhite: "#F5F0EB", gold: "#C4993A", dark: "#1A0A0E",
  darkMid: "#2D1219", muted: "#9E7A82", surface: "#FBF8F5", border: "#E8DDD5",
};
...
const ADMIN = { email:"shirley@benettiangular.com.br", senha:"benetti@2026" };
...
const alertaFollowUp = (lead) => { const d=diasAtraso(lead.followUpData); if(d===null) return null; if(d>0) return {tipo:"atrasado",texto:`${d}d atrasado`}; if(d===0) return {tipo:"hoje",texto:"Hoje!"}; if(d>=-2) return {tipo:"proximo",texto:`Em ${Math.abs(d)}d`}; return null; };
const campoObrigatorio = (etapaId,lead) => { if(["tem_interesse","objecao"].includes(etapaId)&&(!lead.perguntas?.dor||!lead.perguntas?.desejo)) return "Preencha as 3 Perguntas de Ouro antes."; return null; };
...
const CLINICAS_INICIAL = [
  {id:1,nome:"Clínica BM",          email:"bm@clinica.com",      senha:"1234",prefixo:"PAC",status:"ativo",   criadoEm:"2026-01-10",bloqueioMsg:MSG_BLOQUEIO_DEFAULT},
  {id:2,nome:"Dra. Paula Meirelles",email:"paula@clinica.com",   senha:"1234",prefixo:"PAU",status:"ativo",   criadoEm:"2026-02-15",bloqueioMsg:MSG_BLOQUEIO_DEFAULT},
  {id:3,nome:"Dra. Isabela Zanin",  email:"isabela@clinica.com", senha:"1234",prefixo:"ISA",status:"pendente",criadoEm:"2026-06-20",bloqueioMsg:MSG_BLOQUEIO_DEFAULT},
  {id:4,nome:"Dra. Samantha N.",    email:"samantha@clinica.com",senha:"1234",prefixo:"SAM",status:"suspenso",criadoEm:"2026-03-01",bloqueioMsg:MSG_BLOQUEIO_DEFAULT},
];

const LEADS_INICIAIS = [
  {id:1,clinicaId:1,importId:null,codigo:"PAC001",nome:"Camila Rocha",    telefone:"11999990001",origem:"Instagram/DM",   etapa:"em_andamento",  ultimaInteracao:addDias(hoje(),-1),followUpData:hoje(),           indiceFollowUp:0,criadoEm:addDias(hoje(),-1),observacoes:"Interesse em harmonização",perguntas:{dor:"",desejo:"",urgencia:""},historico:[]},
  {id:2,clinicaId:1,importId:null,codigo:"PAC002",nome:"Fernanda Lima",   telefone:"11999990002",origem:"WhatsApp direto",etapa:"tem_interesse",  ultimaInteracao:addDias(hoje(),-3),followUpData:hoje(),           indiceFollowUp:0,criadoEm:addDias(hoje(),-5),observacoes:"Quer ver com o marido",    perguntas:{dor:"Flacidez",desejo:"Bioestimulador",urgencia:"Casamento"},historico:[]},
  {id:3,clinicaId:1,importId:null,codigo:"PAC003",nome:"Beatriz Souza",   telefone:"11999990003",origem:"Instagram/DM",   etapa:"nao_responde",   ultimaInteracao:addDias(hoje(),-3),followUpData:hoje(),           indiceFollowUp:0,criadoEm:addDias(hoje(),-4),observacoes:"",perguntas:{dor:"",desejo:"",urgencia:""},historico:[]},
  {id:4,clinicaId:1,importId:null,codigo:"PAC004",nome:"Juliana Torres",  telefone:"11999990004",origem:"WhatsApp direto",etapa:"consulta_agendada",ultimaInteracao:addDias(hoje(),-1),followUpData:addDias(hoje(),3),indiceFollowUp:0,criadoEm:addDias(hoje(),-3),observacoes:"Consulta em 3 dias",    perguntas:{dor:"Manchas",desejo:"Peeling",urgencia:"Evento"},historico:[]},
  {id:5,clinicaId:1,importId:null,codigo:"PAC005",nome:"Ana Clara Mendes",telefone:"11999990005",origem:"Instagram/DM",   etapa:"novo",           ultimaInteracao:hoje(),            followUpData:hoje(),           indiceFollowUp:0,criadoEm:hoje(),            observacoes:"",perguntas:{dor:"",desejo:"",urgencia:""},historico:[]},
];
...
// ─── LOGIN ─────────────────────────────────────────────────────────────────────
function Login({clinicas,setClinicas,onLogin}){
  ...
  const handleLogin=()=>{
    setErro("");
    if(email===ADMIN.email&&senha===ADMIN.senha){onLogin({admin:true});return;}
    const c=clinicas.find(c=>c.email===email&&c.senha===senha);
    if(!c){setErro("Email ou senha incorretos.");return;}
    if(c.status==="pendente"){setErro("Cadastro em análise. A equipe Cél entrará em contato em breve.");return;}
    if(c.status==="suspenso"){onLogin({suspenso:true,clinica:c});return;}
    onLogin({clinica:c});
  };
  ...
  const handleCadastro=()=>{
    ...
    if(clinicas.find(c=>c.email===email)){setErro("Este email já está cadastrado.");return;}
    const prefixo=nome.split(" ").filter(Boolean).map(p=>p[0]).join("").toUpperCase().substring(0,3)||"CLI";
    setClinicas(cs=>[...cs,{id:Date.now(),nome,email,senha,prefixo,status:"pendente",criadoEm:hoje(),bloqueioMsg:MSG_BLOQUEIO_DEFAULT}]);
  ...
  return(
    <div style={{minHeight:"100vh",background:C.dark,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter',sans-serif",flexDirection:"column",gap:16}}>
      <div style={{background:C.darkMid,border:`1px solid ${C.bordo}44`,borderRadius:16,padding:"44px 40px",width:400,boxShadow:"0 24px 60px rgba(0,0,0,0.5)"}}>
        ...
        {[{label:"Email",value:email,set:setEmail,type:"email",ph:"clinica@email.com"},{label:"Senha",value:senha,set:setSenha,type:"password",ph:"••••••••"}].map(f=>(
        ...
        <button onClick={modo==="login"?handleLogin:handleCadastro} style={{width:"100%",padding:12,marginTop:4,background:`linear-gradient(135deg,${C.bordo},${C.vinho})`,border:"none",borderRadius:8,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer"}}>{modo==="login"?"Entrar":"Solicitar acesso"}</button>
        <div style={{marginTop:20,textAlign:"center"}}><BotaoSuporte small/></div>
        {modo==="login"&&<div style={{color:C.muted,fontSize:11,textAlign:"center",marginTop:14,lineHeight:1.6}}>Demo: bm@clinica.com / 1234<br/>Admin: shirley@benettiangular.com.br / benetti@2026</div>}
      </div>
    </div>
  );
}
...
// ─── TELA SUSPENSA ────────────────────────────────────────────────────────────
function TelaSuspensa({clinica,onVoltar}){
  const msg=encodeURIComponent(`Olá! Sou ${clinica.nome} e gostaria de reativar meu acesso ao ${NOME_APP}.`);
  return(
    <div style={{minHeight:"100vh",background:C.dark,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter',sans-serif",padding:24}}>
      <div style={{background:C.darkMid,border:`1px solid ${C.bordo}44`,borderRadius:16,padding:"44px 40px",maxWidth:440,width:"100%",textAlign:"center",boxShadow:"0 24px 60px rgba(0,0,0,0.5)"}}>
        ...
        <div style={{color:C.muted,fontSize:14,lineHeight:1.8,marginBottom:28}}>{clinica.bloqueioMsg}</div>
        <a href={`https://wa.me/${WHATSAPP_SUPORTE}?text=${msg}`} target="_blank" rel="noreferrer" style={{display:"inline-block",padding:"12px 28px",background:"#25D366",border:"none",borderRadius:8,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",textDecoration:"none",marginBottom:14}}>💬 Falar com a equipe {NOME_APP}</a>
...
// ─── PAINEL ADMIN ─────────────────────────────────────────────────────────────
function PainelAdmin({clinicas,setClinicas,onLogout}){
  const [editandoMsg,setEditandoMsg]=useState(null);
  const [msgTemp,setMsgTemp]=useState("");
  const [busca,setBusca]=useState("");
  const [filtroStatus,setFiltroStatus]=useState("todos");
  const pendentes=clinicas.filter(c=>c.status==="pendente");
  const filtradas=clinicas.filter(c=>{if(filtroStatus!=="todos"&&c.status!==filtroStatus)return false;if(busca&&!c.nome.toLowerCase().includes(busca.toLowerCase())&&!c.email.toLowerCase().includes(busca.toLowerCase()))return false;return true;});
  const atualizar=(id,campos)=>setClinicas(cs=>cs.map(c=>c.id===id?{...c,...campos}:c));
  ...
  return(
    <div style={{minHeight:"100vh",background:C.surface,fontFamily:"'Inter',sans-serif"}}>
      <div style={{background:C.dark,borderBottom:`1px solid ${C.bordo}44`,padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between",height:54}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}><Logo size={32} showName/></div>
      ...
      <div style={{padding:24,maxWidth:1000,margin:"0 auto"}}>
        {pendentes.length>0&&<div style={{background:`${C.gold}11`,border:`1px solid ${C.gold}44`,borderRadius:12,padding:"16px 20px",marginBottom:24,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><div style={{color:C.gold,fontWeight:700,fontSize:14}}>{pendentes.length} cadastro{pendentes.length>1?"s":""} aguardando aprovação</div><div style={{color:C.muted,fontSize:13,marginTop:2}}>{pendentes.map(c=>c.nome).join(", ")}</div></div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:10,marginBottom:24}}>
          {[{label:"Total",valor:clinicas.length,cor:C.bordo},{label:"Ativas",valor:clinicas.filter(c=>c.status==="ativo").length,cor:"#4A9B6F"},{label:"Pendentes",valor:clinicas.filter(c=>c.status==="pendente").length,cor:C.gold},{label:"Suspensas",valor:clinicas.filter(c=>c.status==="suspenso").length,cor:"#C0392B"}].map((m,i)=>(
            <div key={i} style={{background:"#fff",border:`1px solid ${C.border}`,borderRadius:10,padding:"14px 16px",cursor:"pointer"}} onClick={()=>setFiltroStatus(["todos","ativo","pendente","suspenso"][i])}>
        ...
        <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <div style={{display:"flex",gap:6}}>{["todos","ativo","pendente","suspenso"].map(s=><button key={s} onClick={()=>setFiltroStatus(s)} style={{padding:"7px 14px",border:`1px solid ${filtroStatus===s?C.bordo:C.border}`,borderRadius:20,background:filtroStatus===s?`${C.bordo}11`:"#fff",color:filtroStatus===s?C.bordo:C.muted,fontSize:12,fontWeight:filtroStatus===s?700:400,cursor:"pointer",textTransform:"capitalize"}}>{s==="todos"?"Todas":statusLabel[s]||s}</button>)}</div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {filtradas.map(c=>(
            <div key={c.id} style={{background:"#fff",border:`1px solid ${C.border}`,borderLeft:`3px solid ${statusCor[c.status]||C.muted}`,borderRadius:10,padding:"14px 20px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
                <div><div style={{display:"flex",alignItems:"center",gap:10}}><span style={{fontWeight:700,color:C.dark,fontSize:15}}>{c.nome}</span><Badge cor={statusCor[c.status]||C.muted} small>{statusLabel[c.status]||c.status}</Badge></div><div style={{color:C.muted,fontSize:13,marginTop:3}}>{c.email} · Desde {c.criadoEm}</div></div>
                <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                  {c.status==="pendente"&&<button onClick={()=>atualizar(c.id,{status:"ativo"})} style={{padding:"7px 14px",border:"none",borderRadius:8,background:"#4A9B6F",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>✓ Aprovar</button>}
                  {c.status==="ativo"&&<button onClick={()=>atualizar(c.id,{status:"suspenso"})} style={{padding:"7px 14px",border:`1px solid #C0392B44`,borderRadius:8,background:"transparent",color:"#C0392B",fontSize:12,fontWeight:600,cursor:"pointer"}}>Suspender</button>}
                  {c.status==="suspenso"&&<button onClick={()=>atualizar(c.id,{status:"ativo"})} style={{padding:"7px 14px",border:"none",borderRadius:8,background:"#4A9B6F",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>Reativar</button>}
                  <button onClick={()=>{setEditandoMsg(c.id);setMsgTemp(c.bloqueioMsg||MSG_BLOQUEIO_DEFAULT);}} style={{padding:"7px 14px",border:`1px solid ${C.border}`,borderRadius:8,background:"transparent",color:C.muted,fontSize:12,cursor:"pointer"}}>✏ Mensagem</button>
                </div>
              </div>
              {editandoMsg===c.id&&<div style={{marginTop:14,padding:14,background:C.offWhite,borderRadius:8,border:`1px solid ${C.border}`}}>
                <label style={{...lStyle,marginBottom:8}}>Mensagem de bloqueio para {c.nome}</label>
                ...
                <div style={{display:"flex",gap:8,justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    ...
                    <button onClick={()=>{atualizar(c.id,{bloqueioMsg:msgTemp});setEditandoMsg(null);}} style={{padding:"7px 14px",border:"none",borderRadius:8,background:C.bordo,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>Salvar</button>
                  </div>
                </div>
              </div>}
            </div>
...
// ─── MODAL IMPORTAR ───────────────────────────────────────────────────────────
function ModalImportar({onClose,onImportar,clinica,leadsExistentes}){
    ...
    reader.onload=(ev)=>{
      try{
        ...
        const mapeados=rows.map((row,i)=>{
          ...
          const etapaMapeada=MAP_STATUS[statusRaw]||"novo";
          const codRaw=get("cod","codigo","code");
          const codigo=codRaw||gerarCodigo(clinica.prefixo,[...leadsExistentes]);
          const followUpRaw=get("follow-up","followup","prox");
          let followUpData=""; if(followUpRaw){const d=new Date(followUpRaw);if(!isNaN(d))followUpData=d.toISOString().split("T")[0];}
          const dataRegRaw=get("data","registro","criado");
          let criadoEm=hoje(); if(dataRegRaw){const d=new Date(dataRegRaw);if(!isNaN(d))criadoEm=d.toISOString().split("T")[0];}
          return{id:Date.now()+i,clinicaId:clinica.id,importId:null,codigo,nome:get("nome","name","paciente")||`Lead ${i+1}`,telefone:get("telefone","fone","celular","whatsapp"),origem:get("origem","source","canal")||"Outros",etapa:etapaMapeada,ultimaInteracao:criadoEm,followUpData:followUpData||calcProximoFollowUp(etapaMapeada,criadoEm,0)||"",indiceFollowUp:0,criadoEm,observacoes:get("obs","observa","nota","anotacao"),perguntas:{dor:"",desejo:"",urgencia:""},historico:[],_statusOriginal:get("status","etapa","situacao")};
        });
        setPreview(mapeados);
        ...
        {!preview?(
          ...
          <div>
            <div style={{marginBottom:14}}>
              <label style={lStyle}>Nome da importação</label>
              <input style={iStyle} value={nomeImport} onChange={e=>setNomeImport(e.target.value)} placeholder="Ex: Base março 2026"/>
            </div>
            ...
            <div style={{marginTop:14,display:"flex",justifyContent:"flex-end",gap:10}}>
              ...
              <button onClick={()=>{const importId=Date.now();onImportar(preview.map(({_statusOriginal,...l})=>({...l,importId,importNome:nomeImport||"Importação"})));onClose();}} style={{padding:"9px 20px",border:"none",borderRadius:8,background:`linear-gradient(135deg,${C.bordo},${C.vinho})`,color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer"}}>✓ Importar {preview.length} leads</button>
            </div>
          </div>
        )}
      </div>
...
function ModalLead({lead,onClose,onSave,onDelete}){
  ...
  const setPergunta=(k,v)=>setForm(f=>({...f,perguntas:{...f.perguntas,[k]:v}}));
  const handleEtapaChange=(novaEtapa)=>{const erro=campoObrigatorio(novaEtapa,form);if(erro){setErroEtapa(erro);return;}setErroEtapa("");const base=hoje();setForm(f=>({...f,etapa:novaEtapa,ultimaInteracao:base,followUpData:calcProximoFollowUp(novaEtapa,base,0)||f.followUpData,indiceFollowUp:0,historico:[...(f.historico||[]),{data:base,de:f.etapa,para:novaEtapa}]}));};
  const handleRegistrarContato=()=>{const c=CADENCIAS[form.etapa];if(!c?.proximos?.length)return;const novoIdx=form.indiceFollowUp+1;const base=hoje();const atingiu=novoIdx>=c.proximos.length;const proxEtapa=atingiu?proximaEtapaAposCadencia(form.etapa):null;const novoFU=!atingiu?calcProximoFollowUp(form.etapa,base,novoIdx):null;setForm(f=>({...f,ultimaInteracao:base,indiceFollowUp:novoIdx,followUpData:novoFU||f.followUpData,etapa:proxEtapa||f.etapa,historico:[...(f.historico||[]),{data:base,de:f.etapa,para:proxEtapa||f.etapa,nota:atingiu?"Cadência encerrada":`Follow-up ${novoIdx+1}`}]}));};
  const etapaAtual=ETAPAS.find(e=>e.id===form.etapa);
  const cadencia=CADENCIAS[form.etapa];
  const proximos=cadencia?.proximos||[];
  const atingiuFim=proximos.length>0&&form.indiceFollowUp>=proximos.length;
  return(
    <Modal onClose={onClose}>
      ...
      <div style={{padding:24}}>
        {aba==="dados"&&<div style={{display:"grid",gap:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <div><label style={lStyle}>Origem</label><select style={iStyle} value={form.origem} onChange={e=>set("origem",e.target.value)}>{ORIGENS.map(o=><option key={o}>{o}</option>)}</select></div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <div><label style={lStyle}>Última Interação</label><input style={iStyle} type="date" value={form.ultimaInteracao} onChange={e=>set("ultimaInteracao",e.target.value)}/></div>
            <div><label style={lStyle}>Próximo Follow-up</label><input style={iStyle} type="date" value={form.followUpData||""} onChange={e=>set("followUpData",e.target.value)}/></div>
          </div>
        ...
        {aba==="regras"&&<div>
          <div style={{padding:16,borderRadius:10,background:etapaAtual?.cor+"11",border:`1px solid ${etapaAtual?.cor}33`,marginBottom:20}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              ...
              {proximos.length>0&&<Badge cor={etapaAtual?.cor}>{atingiuFim?"Fim da cadência":`Contato ${form.indiceFollowUp+1} de ${proximos.length}`}</Badge>}
            </div>
            {proximos.length>0&&<div style={{marginTop:14}}><div style={{color:C.muted,fontSize:11,marginBottom:8,textTransform:"uppercase",letterSpacing:"0.04em"}}>Sequência</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{proximos.map((d,i)=><div key={i} style={{padding:"4px 12px",borderRadius:20,fontSize:12,fontWeight:600,background:i<form.indiceFollowUp?C.darkMid:i===form.indiceFollowUp?etapaAtual?.cor:"#fff",color:i<form.indiceFollowUp?C.muted:i===form.indiceFollowUp?"#fff":C.muted,border:`1px solid ${i===form.indiceFollowUp?etapaAtual?.cor:C.border}`,textDecoration:i<form.indiceFollowUp?"line-through":"none"}}>{i<form.indiceFollowUp?"✓ ":""}{d}d</div>)}<div style={{padding:"4px 12px",borderRadius:20,fontSize:12,background:"#fff",border:`1px solid ${C.border}`,color:C.muted}}>→ {proximaEtapaAposCadencia(form.etapa)?ETAPAS.find(e=>e.id===proximaEtapaAposCadencia(form.etapa))?.label:"Agendamento"}</div></div></div>}
        ...
        {aba==="perguntas"&&<div>
          ...
          {[{key:"dor",label:"1. Qual é a DOR?",hint:"O que está incomodando?",cor:C.bordo},{key:"desejo",label:"2. Qual é o DESEJO?",hint:"Como ela quer se sentir depois?",cor:"#5B7FA6"},{key:"urgencia",label:"3. Qual é a URGÊNCIA?",hint:"Evento, prazo ou motivo que acelera?",cor:C.gold}].map(p=><div key={p.key} style={{marginBottom:14,padding:14,borderRadius:10,border:`1px solid ${form.perguntas[p.key]?p.cor+"44":C.border}`,background:form.perguntas[p.key]?p.cor+"08":C.offWhite}}><label style={{...lStyle,color:p.cor,marginBottom:3}}>{p.label}</label><div style={{color:C.muted,fontSize:12,marginBottom:7,fontStyle:"italic"}}>{p.hint}</div><textarea style={{...iStyle,height:56,resize:"none"}} value={form.perguntas[p.key]} onChange={e=>setPergunta(p.key,e.target.value)} placeholder="Escreva aqui..."/></div>)}
          {form.perguntas.dor&&form.perguntas.desejo&&<div style={{padding:14,background:`${C.gold}11`,border:`1px solid ${C.gold}33`,borderRadius:10}}><div style={{color:C.gold,fontWeight:700,fontSize:13,marginBottom:6}}>✨ Espelho</div><div style={{color:C.dark,fontSize:13,lineHeight:1.7}}>"Então, você está sentindo <strong>{form.perguntas.dor}</strong> e quer <strong>{form.perguntas.desejo}</strong>{form.perguntas.urgencia?`, especialmente porque ${form.perguntas.urgencia}`:""}. Faz sentido?"</div></div>}
        </div>}
        {aba==="historico"&&<div>
      ...
      <div style={{padding:"14px 24px",borderTop:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",background:C.surface,borderRadius:"0 0 12px 12px"}}>
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <button onClick={()=>{onSave(form);onClose();}} style={{padding:"8px 20px",border:"none",borderRadius:8,background:`linear-gradient(135deg,${C.bordo},${C.vinho})`,color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer"}}>Salvar</button>
        </div>
      </div>
    </Modal>
  );
...
// ─── CRM ─────────────────────────────────────────────────────────────────────
function CRM({clinica,onLogout}){
  const [leads,setLeads]=useState(LEADS_INICIAIS.filter(l=>l.clinicaId===clinica.id));
  ...
  const meuLeads=leads;
  const leadsFiltrados=meuLeads.filter(l=>{if(filtroEtapa!=="todos"&&l.etapa!==filtroEtapa)return false;if(busca&&!l.nome?.toLowerCase().includes(busca.toLowerCase())&&!l.codigo?.toLowerCase().includes(busca.toLowerCase())&&!l.telefone?.includes(busca))return false;return true;});
  const followupsHoje=meuLeads.filter(l=>{const d=diasAtraso(l.followUpData);return d!==null&&d>=0;}).sort((a,b)=>diasAtraso(b.followUpData)-diasAtraso(a.followUpData));

  const leadsNoPeriodo=usarPersonalizado&&dataInicio&&dataFim
    ? meuLeads.filter(l=>l.criadoEm>=dataInicio&&l.criadoEm<=dataFim)
    : filtrarPorPeriodo(meuLeads,periodo);

  // Importações únicas
  const importacoes=[...new Map(meuLeads.filter(l=>l.importId).map(l=>[l.importId,{id:l.importId,nome:l.importNome||"Importação",qtd:meuLeads.filter(x=>x.importId===l.importId).length}])).values()];

  const totalPorEtapa=(id)=>meuLeads.filter(l=>l.etapa===id).length;
  const salvarLead=(form)=>setLeads(ls=>ls.map(l=>l.id===form.id?{...form}:l));
  const excluirLead=(id)=>{setLeads(ls=>ls.filter(l=>l.id!==id));setLeadSelecionado(null);};
  const excluirImportacao=(importId)=>setLeads(ls=>ls.filter(l=>l.importId!==importId));
  const criarLead=(form)=>{const codigo=form.codigo||gerarCodigo(clinica.prefixo,meuLeads);setLeads(ls=>[...ls,{...form,id:Date.now(),clinicaId:clinica.id,codigo,importId:null,criadoEm:hoje(),ultimaInteracao:hoje(),followUpData:calcProximoFollowUp(form.etapa,hoje(),0)||hoje(),indiceFollowUp:0,historico:[]}]);setNovoLead(false);};
  const importarLeads=(novos)=>setLeads(ls=>[...ls,...novos]);
  const leadVazio={id:null,codigo:"",nome:"",telefone:"",origem:"Instagram/DM",etapa:"novo",ultimaInteracao:hoje(),followUpData:hoje(),indiceFollowUp:0,criadoEm:hoje(),importId:null,observacoes:"",perguntas:{dor:"",desejo:"",urgencia:""},historico:[]};
  ...
  return(
    <div style={{minHeight:"100vh",background:C.surface,fontFamily:"'Inter',sans-serif",display:"flex",flexDirection:"column"}}>
      {/* Header */}
      <div style={{background:C.dark,borderBottom:`1px solid ${C.bordo}44`,padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between",height:54}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}><Logo size={30} showName/></div>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          <BotaoSuporte small/>
          <button onClick={onLogout} style={{background:"transparent",border:`1px solid ${C.bordo}55`,color:C.muted,padding:"5px 12px",borderRadius:6,fontSize:12,cursor:"pointer"}}>Sair</button>
        </div>
      </div>
      {/* Nav */}
          ...
          {importacoes.length>0&&<div style={{marginBottom:20,padding:14,background:"#fff",border:`1px solid ${C.border}`,borderRadius:10}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {importacoes.map(imp=><div key={imp.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 12px",background:C.offWhite,border:`1px solid ${C.border}`,borderRadius:8}}>
                <span style={{fontSize:13,color:C.dark}}>📊 {imp.nome}</span>
                <span style={{fontSize:11,color:C.muted}}>{imp.qtd} leads</span>
                <button onClick={()=>{if(window.confirm(`Excluir a importação "${imp.nome}" e todos os ${imp.qtd} leads dela?`))excluirImportacao(imp.id);}} style={{background:"none",border:"none",color:"#C0392B",cursor:"pointer",fontSize:14,lineHeight:1,padding:"0 2px"}}>✕</button>
              </div>)}
            </div>
          </div>}
      ...
      {importando&&<ModalImportar onClose={()=>setImportando(false)} onImportar={importarLeads} clinica={clinica} leadsExistentes={meuLeads}/>}
    </div>
  );
}
...
function filtrarPorPeriodo(leads,periodo){
  const agora=new Date(); const ini=new Date();
  if(periodo==="hoje") ini.setHours(0,0,0,0);
  else if(periodo==="semana") ini.setDate(agora.getDate()-7);
  else if(periodo==="mes") ini.setDate(1);
  else if(periodo==="trim") ini.setMonth(agora.getMonth()-3);
  else if(periodo==="ano") ini.setMonth(0,1);
  else return leads;
  return leads.filter(l=>new Date(l.criadoEm)>=ini);
}

// ─── APP ROOT ─────────────────────────────────────────────────────────────────
export default function App(){
  const [clinicas,setClinicas]=useState(CLINICAS_INICIAL);
  const [sessao,setSessao]=useState(null);
  if(!sessao) return <Login clinicas={clinicas} setClinicas={setClinicas} onLogin={setSessao}/>;
  if(sessao.admin) return <PainelAdmin clinicas={clinicas} setClinicas={setClinicas} onLogout={()=>setSessao(null)}/>;
  if(sessao.suspenso) return <TelaSuspensa clinica={sessao.clinica} onVoltar={()=>setSessao(null)}/>;
  return <CRM clinica={sessao.clinica} onLogout={()=>setSessao(null)}/>;
}
